...
def process_car_rental_data(data_date):
    ...
    spark = SparkSession.builder \
        .appName("CarRentalBatchIngestion") \
        .config("spark.jars.packages", ",".join(spark_jars_packages)) \
        .getOrCreate()

    # Define lake file path based on the date argument
    hdfs_host = "localhost:9000"
    hdfs_file_path = f"hdfs://{hdfs_host}/car_rental_batch_ingestion/car_rental_daily_data/car_rental_{data_date}.json"
    
    # Read raw JSON data
    raw_df = spark.read.option("multiline", "true").json(hdfs_file_path)
    ...
    # Join transformed raw data with dimension tables to get dimension keys
    # Join with car_dim
